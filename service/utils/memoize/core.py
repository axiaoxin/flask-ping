...
PROTOCOL_INDEX, CREATION_INDEX, EXPIRY_INDEX, ETAG_INDEX, VALUE_INDEX = list(
    range(5))
...
class Memoizer(object):
    ...
    def get(self, key, func=None, args=(), kwargs=None, **opts):
        ...
        if max_age is not None:
            expiry = min(
                x for x in (expiry, creation + max_age) if x is not None)

        # Need to be careful as this is the only place where we do not use the
        # lovely index constants.
        store[key] = (CURRENT_PROTOCOL_VERSION, creation, expiry,
                      opts.get('etag'), value)
...
class MemoizedFunction(object):
    ...
    def __repr__(self):
        return '<%s of %s via %s>' % (self.__class__.__name__, self.func,
                                      self.cache)
    ...
    def __call__(self, *args, **kwargs):
        ...
        return self.cache.get(
            self.key(args, copy_kwargs), self.func, args, kwargs, **self.opts)

    def get(self, args=(), kwargs=None, **opts):
        args, kwargs = self._expand_args(args, kwargs)
        self._expand_opts(opts)
        return self.cache.get(
            self.key(args, kwargs), self.func, args, kwargs, **opts)
